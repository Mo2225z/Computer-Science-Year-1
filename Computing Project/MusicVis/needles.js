	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				var h = ??
				*/
                var x = 30 + i * width/2.1;
                var y = 45 + j * height/2.1;
                var w = this.plotWidth - this.pad/5;
                var h = this.plotHeight - this.pad/9 + 7;
				...
				//add on the ticks
				// ??
                this.ticks(x + this.plotWidth/2,y + this.plotHeight,this.frequencyBins[i + j * 2]);
